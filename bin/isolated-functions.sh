...
__elog_base() {
	...
	case "${1}" in
		INFO|WARN|ERROR|LOG|QA)
			...
			;;
		*)
			__vecho -e "${PORTAGE_COLOR_BAD}${PORTAGE_PREFIX_ERR}${PORTAGE_COLOR_NORMAL} Invalid use of internal function __elog_base(), next message will not be logged"
			return 1
			;;
	esac
...
eqawarn() {
	...
	echo -e "$@" | while read -r ; do
		echo "${PORTAGE_COLOR_QAWARN}${PORTAGE_PREFIX_QAWARN}${PORTAGE_COLOR_NORMAL} ${REPLY}" >&2
	done
	LAST_E_CMD="eqawarn"
	return 0
}
...
elog() {
	...
	[[ ${RC_ENDCOL} != "yes" && ${LAST_E_CMD} == "ebegin" ]] && echo >&2
	echo -e "$@" | while read -r ; do
		echo "${PORTAGE_COLOR_LOG}${PORTAGE_PREFIX_LOG}${PORTAGE_COLOR_NORMAL} ${REPLY}" >&2
	done
	LAST_E_CMD="elog"
	return 0
}
...
einfo() {
	...
	[[ ${RC_ENDCOL} != "yes" && ${LAST_E_CMD} == "ebegin" ]] && echo >&2
	echo -e "$@" | while read -r ; do
		echo "${PORTAGE_COLOR_INFO}${PORTAGE_PREFIX_INFO}${PORTAGE_COLOR_NORMAL} ${REPLY}" >&2
	done
	LAST_E_CMD="einfo"
	return 0
}
...
einfon() {
	__elog_base INFO "$*"
	[[ ${RC_ENDCOL} != "yes" && ${LAST_E_CMD} == "ebegin" ]] && echo >&2
	echo -ne "${PORTAGE_COLOR_INFO}${PORTAGE_PREFIX_INFO}${PORTAGE_COLOR_NORMAL} $*" >&2
	LAST_E_CMD="einfon"
	return 0
}
...
ewarn() {
	...
	[[ ${RC_ENDCOL} != "yes" && ${LAST_E_CMD} == "ebegin" ]] && echo >&2
	echo -e "$@" | while read -r ; do
		echo "${PORTAGE_COLOR_WARN}${PORTAGE_PREFIX_WARN}${PORTAGE_COLOR_NORMAL} ${RC_INDENTATION}${REPLY}" >&2
	done
	LAST_E_CMD="ewarn"
	return 0
}
...
eerror() {
	...
	[[ ${RC_ENDCOL} != "yes" && ${LAST_E_CMD} == "ebegin" ]] && echo >&2
	echo -e "$@" | while read -r ; do
		echo "${PORTAGE_COLOR_ERR}${PORTAGE_PREFIX_ERR}${PORTAGE_COLOR_NORMAL} ${RC_INDENTATION}${REPLY}" >&2
	done
	LAST_E_CMD="eerror"
	return 0
}
...
__set_colors() {
	...
	if [[ -n ${PORTAGE_OUTPUT_PREFIXES} ]] ; then
		eval ${PORTAGE_OUTPUT_PREFIXES}
	else
		PORTAGE_PREFIX_ERR='[EE]'
		PORTAGE_PREFIX_INFO='[..]'
		PORTAGE_PREFIX_LOG='[II]'
		PORTAGE_PREFIX_QAWARN='[QA]'
		PORTAGE_PREFIX_WARN='[WW]'
	fi
}
